...
if ( `where fzf` != "" ) then
    if ( `where git` != "" ) then
        if ( `where fd` != "" ) then
            setenv FZF_DEFAULT_COMMAND 'git --no-pager ls-files -co --exclude-standard || fd -E "*.spl" -E "*.aux" -E "*.out" -E "*.o" -E "*.pyc" -E "*.gz" -E "*.pdf" -E "*.sw" -E "*.swp" -E "*.swap" -E "*.com" -E "*.exe" -E "*.so" -E "/cache/" -E "/__pycache__/" -E "/tmp/" -E ".git/" -E ".svn/" -E ".xml" -E "*.bin" -E "*.7z" -E "*.dmg" -E "*.gz" -E "*.iso" -E "*.jar" -E "*.rar" -E "*.tar" -E "*.zip" -E "TAGS" -E "tags" -E "GTAGS" -E "COMMIT_EDITMSG" --type f --hidden --follow --color never . . '
